<template>
  <div style="margin-top: 50px">
    <el-form :model="value" ref="bookSaleForm" label-width="120px" style="width: 600px" size="small">
      ...
      <el-form-item v-show="value.promotionType===3">
        <el-table :data="value.bookLadderList"
          ...
          <el-table-column
            ...
            label="操作">
            <template slot-scope="scope">
              <el-button type="text" @click="handleRemoveBookLadder(scope.$index, scope.row)">删除</el-button>
              <el-button type="text" @click="handleAddBookLadder(scope.$index, scope.row)">添加</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-form-item>
      <el-form-item v-show="value.promotionType===4">
        <el-table :data="value.bookFullReductionList"
                  style="width: 80%" border>
          <el-table-column
            label="满"
            align="center"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      handleRemoveBookLadder(index, row) {
        let bookLadderList = this.value.bookLadderList;
        if (bookLadderList.length === 1) {
          bookLadderList.pop();
          bookLadderList.push({
            count: 0,
            discount: 0,
          ...
          bookLadderList.splice(index, 1);
        }
      },
      handleAddBookLadder(index, row) {
        let bookLadderList = this.value.bookLadderList;
        if (bookLadderList.length < 3) {
          bookLadderList.push({
            count: 0,
            discount: 0,
            price: 0
      ...
      handleRemoveFullReduction(index, row) {
        let fullReductionList = this.value.bookFullReductionList;
