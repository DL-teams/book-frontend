  ...
  <div class="app-container">
        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <p>
              <el-button
                size="mini"
                @click="handleShowBook(scope.$index, scope.row)">查看
              </el-button>
              <el-button
                size="mini"
                @click="handleUpdateBook(scope.$index, scope.row)">编辑
              </el-button>
            </p>
            <p>
              <el-button
    ...
    <el-dialog
      ...
      <el-table style="width: 100%;margin-top: 20px"
        ...
        <el-table-column
          v-for="(item,index) in editSkuInfo.bookAttr"
          :label="item.name"
          :key="item.id"
          align="center">
          <template slot-scope="scope">
...
export default {
  ...
  watch: {
    selectBookCateValue: function (newValue) {
      if (newValue != null && newValue.length == 2) {
        this.listQuery.bookCategoryId = newValue[1];
      } else {
        this.listQuery.bookCategoryId = null;
      }
  ...
  methods: {
    ...
    handleShowSkuEditDialog(index, row) {
      ...
      this.editSkuInfo.bookId = row.id;
      this.editSkuInfo.bookSn = row.bookSn;
      this.editSkuInfo.bookAttributeCategoryId = row.bookAttributeCategoryId;
      this.editSkuInfo.keyword = null;
      fetchSkuStockList(row.id, {keyword: this.editSkuInfo.keyword}).then(response => {
        this.editSkuInfo.stockList = response.data;
      });
      if (row.bookAttributeCategoryId != null) {
        fetchBookAttrList(row.bookAttributeCategoryId, {type: 0}).then(response => {
          this.editSkuInfo.bookAttr = response.data.list;
        });
      }
    },
    handleSearchEditSku() {
      fetchSkuStockList(this.editSkuInfo.bookId, {keyword: this.editSkuInfo.keyword}).then(response => {
        this.editSkuInfo.stockList = response.data;
      });
    },
    handleEditSkuConfirm() {
      ...
      this.$confirm('是否要进行修改', '提示', {
        ...
        updateSkuStockList(this.editSkuInfo.bookId, this.editSkuInfo.stockList).then(response => {
          this.$message({
            message: '修改成功',
            type: 'success',
            duration: 1000
    ...
    handleAddBook() {
      this.$router.push({path: '/bms/addBook'});
    },
    handleBatchOperate() {
    ...
    handleUpdateBook(index, row) {
      this.$router.push({path: '/bms/updateBook', query: {id: row.id}});
    },
    handleShowBook(index, row) {
      console.log("handleShowBook", row);
    },
