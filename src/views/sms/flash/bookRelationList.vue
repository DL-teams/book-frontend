<template> 
  <div class="app-container">
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button size="mini" class="btn-add" @click="handleSelectBook()" style="margin-left: 20px">添加</el-button>
    </el-card>
    <div class="table-container">
      <el-table ref="bookRelationTable"
        ...
        <el-table-column label="货号" width="140" align="center">
          <template slot-scope="scope">NO.{{scope.row.book.bookSn}}</template>
        </el-table-column>
        ...
        <el-table-column label="剩余数量" width="100" align="center">
          <template slot-scope="scope">{{scope.row.book.stock}}</template>
        </el-table-column>
    ...
    <el-dialog title="编辑秒杀商品信息"
      ...
      width="40%">
      <el-form :model="flashBookRelation"
               ref="flashBookRelationForm"
               label-width="150px" size="small">
        <el-form-item label="商品名称：">
          <span>{{flashBookRelation.book.name}}</span>
        </el-form-item>
        <el-form-item label="货号：">
          <span>NO.{{flashBookRelation.book.bookSn}}</span>
        </el-form-item>
        <el-form-item label="商品价格：">
          <span>￥{{flashBookRelation.book.price}}</span>
        </el-form-item>
        <el-form-item label="秒杀价格：">
          <el-input v-model="flashBookRelation.flashPromotionPrice" class="input-width">
            <template slot="prepend">￥</template>
          </el-input>
        </el-form-item>
        <el-form-item label="剩余数量：">
          <span>{{flashBookRelation.book.stock}}</span>
        </el-form-item>
        <el-form-item label="秒杀数量：">
          <el-input v-model="flashBookRelation.flashPromotionCount" class="input-width"></el-input>
        </el-form-item>
        <el-form-item label="限购数量：">
          <el-input v-model="flashBookRelation.flashPromotionLimit" class="input-width"></el-input>
        </el-form-item>
        <el-form-item label="排序：">
          <el-input v-model="flashBookRelation.sort" class="input-width"></el-input>
        </el-form-item>
      </el-form>
...
<script>
  import {fetchList,createFlashBookRelation,deleteFlashBookRelation,updateFlashBookRelation} from '@/api/flashBookRelation';
  ...
  export default {
    name:'flashPromotionBookRelationList',
    data() {
      return {
    ...
    methods:{
      ...
      handleUpdate(index,row){
        ...
        this.flashBookRelation = Object.assign({},row);
      },
      handleDelete(index,row){
        this.$confirm('是否要删除该商品?', '提示', {
          ...
          deleteFlashBookRelation(row.id).then(response => {
            this.$message({
              type: 'success',
              message: '删除成功!'
            });
      ...
      handleSelectDialogConfirm(){
        ...
        this.$confirm('使用要进行添加操作?', '提示', {
          ...
          createFlashBookRelation(selectBooks).then(response=>{
      ...
      handleEditDialogConfirm(){
        this.$confirm('是否要确认?', '提示', {
          ...
          type: 'warning'
            ...
            updateFlashBookRelation(this.flashBookRelation.id,this.flashBookRelation).then(response => {
              this.$message({
                message: '修改成功！',
                type: 'success'
              });
